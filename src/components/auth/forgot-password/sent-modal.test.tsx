...
describe('SentModal', () => {
  ...
  it('does not render when open is false', () => {
    render(
      <SentModal open={false} onClose={onClose} sentEmail={sentEmail} onResend={onResend} />
    );
  ...
  it('renders modal with correct content when open', () => {
    render(
      <SentModal open={true} onClose={onClose} sentEmail={sentEmail} onResend={onResend} />
    );
    expect(screen.getByText(/email is sent!/i)).toBeInTheDocument();
    expect(screen.getByText(sentEmail)).toBeInTheDocument();
    expect(screen.getByText(/please check your inbox for reset password instruction/i)).toBeInTheDocument();
